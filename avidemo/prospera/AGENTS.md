...
- **Where:** `src/proxy.ts`. On Next.js 16 (this app) `proxy.ts` is the supported filename — it replaced `middleware.ts`. Do **not** add a `middleware.ts` that re-exports it; that was the Next.js 15 workaround and is no longer needed.
...
- **Edge middleware:** Implemented in **`src/proxy.ts`**. Next.js 16 renamed the middleware entry point to `proxy.ts`, so this file runs as-is — no `middleware.ts` shim is required (or wanted).
...
- **Matcher:** Exclude API routes, `_next/`, sitemap, robots, healthz, Sitecore paths, and static assets so middleware does not run on every static request. The matcher is defined in `config` in `proxy.ts`.
