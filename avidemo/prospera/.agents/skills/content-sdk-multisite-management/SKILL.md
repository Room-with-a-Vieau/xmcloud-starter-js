...
- Site list: `.sitecore/sites.json`. Proxy logic lives in `src/proxy.ts`, which Next.js 16 runs directly (no `middleware.ts` shim). Keep chain order: LocaleProxy → AppRouterMultisiteProxy → RedirectsProxy → PersonalizeProxy. Exclude API, _next, static assets in the matcher.
...
- **Proxy:** Implemented in `src/proxy.ts`. On Next.js 16 `proxy.ts` is the middleware entry point that replaced `middleware.ts`; do not add a `middleware.ts` re-export.
